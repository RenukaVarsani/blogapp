...
const random_string_config_1 = require("./random-string-config");
const random_string_builder_1 = require("./random-string-builder");
const random_string_generator_1 = require("./random-string-generator");
const string_replacer_1 = require("./string-replacer");
class Str {
    /**
    ...
    equalsIgnoreCase(value) {
        return this.lower().equals(new Str(value).lower().get());
    }
    /**
     ...
     * @returns {String}
     */
    random(lengthOrCallback) {
        const config = new random_string_config_1.RandomStringConfig();
        const builder = new random_string_builder_1.RandomStringBuilder(config);
        if (!lengthOrCallback) {
    ...
    generateRandom(config) {
        return new random_string_generator_1.RandomStringGenerator(Str, config).generate();
    }
    replace(searchValue, replacer) {
        if (replacer == null) {
            return new string_replacer_1.StringReplacer(this, searchValue);
        }
        return new Str(this.value.replace(searchValue, replacer));
    }
    replaceAll(searchValue, replace) {
        if (replace == null) {
            return new string_replacer_1.StringReplacer(this, searchValue).all();
        }
        const replacer = new RegExp(searchValue, 'g');
        return new Str(this.value.replace(replacer, replace));
    }
    replaceLast(searchValue, replace) {
        if (replace == null) {
            return new string_replacer_1.StringReplacer(this, searchValue).last();
        }
        return this.notContains(searchValue.toString())
            ? this
            : new Str(this.beforeLast(searchValue.toString()).get() + replace + this.afterLast(searchValue.toString()).get());
    }
    /**
     ...
     */
    reverse() {
        return new Str(this.chars().reverse().join(''));
    }
    /**
   ...
   * from the left of the string if you pass along a value.
     ...
     */
    slice(start, end) {
        return new Str(this.value.slice(start, end));
    }
    /**
     ...
     */
    isUuid() {
        return (0, uuid_1.validate)(this.value);
    }
    /**
